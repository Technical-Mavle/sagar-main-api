...
async def understand_and_execute(request: PromptRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        You MUST call the discover_and_correlate function with the appropriate parameters. Do not provide any explanation or additional text - just make the function call.
        """
        
        # 3. Call the Gemini API
        print(f"Calling Gemini API with prompt: {context_prompt[:200]}...")
        ...
        try:
            ...
            if not response.candidates or len(response.candidates) == 0:
                raise HTTPException(status_code=400, detail="No response generated from Gemini API.")
            
            candidate = response.candidates[0]
            if not candidate.content or not candidate.content.parts:
                raise HTTPException(status_code=400, detail="No content parts in Gemini response.")
            
            # Look for function call in any of the parts
            function_call = None
            ...
            if not function_call:
                # If no function call found, try to extract from text response
                text_response = ""
                for part in candidate.content.parts:
                ...
                raise HTTPException(
                    status_code=400, 
                    detail=f"Gemini did not generate a function call. Response: {text_response[:200]}..."
                )
            
            if function_call.name != "discover_and_correlate":
                raise HTTPException(
                    status_code=400, 
                    detail=f"Expected 'discover_and_correlate' function call, got '{function_call.name}'"
                )
